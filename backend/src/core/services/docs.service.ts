...
export class DocsService {
  ...
  async savePlanAsHtml(
    ...
    this.logger.info('Generating HTML from plan', { sourceFile: filename });

    const htmlFolderPath = this.fileUtil.getHtmlFolderPath(__dirname);
    await this.fileUtil.ensureDirectoryExists(htmlFolderPath);

    const prompt = this.promptService.buildHtmlGenerationPrompt(filename, question, plan);
    const htmlContent = await this.llm.generateContent(prompt, 'gemini-2.5-pro');
